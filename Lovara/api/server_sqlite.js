// API Server for Lovara application using SQLite
...
// Import SQLite database utilities and models
const { initDb } = require('../connection_sqlite');
const User = require('../db/models/User_sqlite');
const Match = require('../db/models/Match_sqlite');
const Message = require('../db/models/Message_sqlite');
const DatabaseUtils = require('../db/utils_sqlite');
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const newUser = await User.create({
      ...
      location: JSON.stringify({ lat: -6.200000, lng: 106.816666 }), // Jakarta coordinates as default
    ...
    await require('./db/connection_sqlite').run(`
    ...
    if (interests && Array.isArray(interests)) {
      for (const interestName of interests) {
        ...
        let interestResult = await require('./db/connection_sqlite').query('SELECT id FROM interests WHERE name = ?', [interestName]);
        let interestId;
        
        if (interestResult.length === 0) {
          const newInterestId = crypto.randomUUID();
          await require('./db/connection_sqlite').run(`
            INSERT INTO interests (id, name) VALUES (?, ?)
          `, [newInterestId, interestName]);
        ...
        const userInterestId = crypto.randomUUID();
        await require('./db/connection_sqlite').run(`
          INSERT INTO user_interests (id, user_id, interest_id) VALUES (?, ?, ?)
        `, [userInterestId, newUser.id, interestId]);
      }
    }
...
app.get('/api/conversations/:conversationId/messages', authenticateToken, async (req, res) => {
  try {
    ...
    `;
    const convCheckResult = await require('./db/connection_sqlite').query(convCheckSql, [conversationId, req.user.id, req.user.id]);
...
app.get('/api/events', authenticateToken, async (req, res) => {
  try {
    const sql = `
      ...
      LIMIT 20
    `;
    const result = await require('./db/connection_sqlite').query(sql);
    res.json(result);
  } catch (error) {
...
app.post('/api/events', authenticateToken, async (req, res) => {
  try {
    ...
    const sql = `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    await require('./db/connection_sqlite').run(sql, [eventId, title, description, date, time, location, req.user.id]);
    
    // Get the created event
    const result = await require('./db/connection_sqlite').query('SELECT * FROM events WHERE id = ?', [eventId]);
...
app.post('/api/events/:eventId/join', authenticateToken, async (req, res) => {
  try {
    ...
    const checkSql = 'SELECT id FROM event_attendees WHERE event_id = ? AND user_id = ?';
    const checkResult = await require('./db/connection_sqlite').query(checkSql, [eventId, req.user.id]);
    ...
    `;
    await require('./db/connection_sqlite').run(joinSql, [joinId, eventId, req.user.id]);
